...
const UserDashboard = () => {
    ...
    const loadTicket = async (bookingId) => {
        ...
        setLoadingTicket(bookingId);

        try {

            const { data } = await api.get(`/bookings/${bookingId}/ticket`);

            setTickets((prev) => ({ ...prev, [bookingId]: data }));

        } catch (error) {

            alert(error.response?.data?.message || 'Could not load ticket');
                                        ...
                                        {booking.status === 'confirmed' && (
                                            ...
                                            <>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => loadTicket(booking._id)}

                                                    disabled={loadingTicket === booking._id}

                                                    className="w-full flex items-center justify-center gap-2 bg-gray-900 text-white text-sm font-bold py-2.5 px-4 rounded-lg hover:bg-black transition"

                                                >

                                                    <FaQrcode />

                                                    {loadingTicket === booking._id ? 'Loading...' : tickets[booking._id] ? 'Hide QR Ticket' : 'Show QR Ticket'}
                                                ...
                                                {tickets[booking._id] && (
                                                    ...
                                                    <TicketQR
                                                        ...
                                                        token={tickets[booking._id].token}

                                                        eventTitle={booking.eventId.title}

                                                        attendeeName={user?.name}

                                                        checkedIn={tickets[booking._id].checkedIn}
