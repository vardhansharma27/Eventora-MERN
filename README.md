...
> **Production (Render)**: Use **Brevo HTTP API** — set `BREVO_API_KEY` and `BREVO_SENDER_EMAIL` (your Gmail verified in Brevo Senders). Render blocks outbound SMTP ports, so SMTP (`BREVO_SMTP_*`) will time out. Create an API key in Brevo → SMTP & API → API keys.
